...
1. **Load selling plans for this product** — pulls the plans your subscription app created in Shopify **per variant** (selling plan groups are attached to variants: on Body Wrinkle Cream the 1-jar variant carries "Every 2 Months · 5% off", the 2- and 3-jar variants "Every 3 Months · 5% off"). Each card is wired to the plan(s) of **its own variant** — never guessed by position — and the per-card select only offers plans that Shopify allows for that variant (a mismatch or a variant without a plan is flagged in red).
...
3. **Everything is editable in one place** (Commerce tab, subscription block): per card — plan, delivery line, offer line, **buy button text**, **small print under the button**, checkmark lines; for the buy box — the **disclaimer/terms under the cards**, footnote, heading and the small wording ("per delivery", "then", default button labels…); plus the **sticky CTA bar text and button** (also in Page settings). FAQ items and the benefits row are ordinary sections.
4. Switching on subscription mode applies editable presets: button labels "Subscribe & save", a **delivery line** and an **offer line** on every card, the **subscription terms** under the cards (recurring-payment disclosure — keep it honest), 4–5 **FAQ** items (how it works, skip/pause/cancel, when am I charged, do I keep the price, what happens after the trial/intro), a **"Why subscribers get the best results"** icon row after the pricing, and the sticky-bar button. Everything is normal section content — edit it anywhere. Switching back to one-time removes what the presets added.
5. On the store: buttons add the line **with its selling plan** (`selling_plan`), then follow your add-to-cart behaviour (Shop All + drawer by default; "straight to checkout" goes through the cart because Shopify permalinks can't carry subscriptions). **Live prices** come from the plan itself per market — subscriber price, strikethrough one-time price, "first delivery … then … per delivery" for intro/trial plans, per-unit and you-save math. The in-app preview derives the same from your manual price × the plan's %.
6. If a market or variant doesn't offer the plan, the card falls back to a one-time card (default) or is hidden — your choice.
7. AI copy and image prompts automatically get a "this is a subscription page" brief; translations pick up the new lines like any other text.
