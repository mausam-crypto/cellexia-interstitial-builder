...
- `api.products` `sellingPlans {handle|productId}` → validated Admin GraphQL (`sellingPlanGroups → sellingPlans → billing/delivery/pricingPolicies` + `variants → sellingPlanGroups { id }`, needs `read_products` only) → `{ plans (with groupId), variantPlans: { variantId: planIds[] } }`. **Selling plan groups are attached per variant** (Body Wrinkle Cream: 1 jar → "Every 2 months", 2/3 jars → "Every 3 months"); `autoWireCardPlans()` wires each card to a plan of its own variant (keeps a valid one, else the variant's first, else "" → warning) and `plansForVariant()` restricts the per-card select. Position-based wiring caused the 1×/2×/3× frequency mix-up and is gone.
- Per-card `belowButton` (small print under the button, follows the subscription in the market fallback) and `buttonLabel`/`checks` are exposed in `SubscriptionControls` next to the plan; section-level terms/footnote/heading/wording and the sticky bar text/button too.
