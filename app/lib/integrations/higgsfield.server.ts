...
function buildBody(model: string, opts: HiggsfieldOpts): Record<string, unknown> {
  const aspect_ratio = normalizeAspectRatio(opts.aspectRatio, "4:3");
  if (model === "nano-banana") {
    return { prompt: opts.prompt, aspect_ratio, num_images: 1, output_format: "jpeg" };
  }
  if (model === "higgsfield-ai/soul/standard") {
    return { prompt: opts.prompt, aspect_ratio, resolution: opts.resolution || "2K", num_images: 1 };
  }
  // soul/reference, flux, reve and anything else: minimal common body
  return { prompt: opts.prompt, aspect_ratio };
}
...
export async function higgsfieldGenerateImage(opts: HiggsfieldOpts): Promise<{ url: string; requestId: string }> {
  ...
  // 1) Submit
  const submitRes = await fetch(`${BASE_URL}/${model}`, { method: "POST", headers, body: JSON.stringify(buildBody(model, opts)) });
  const submitText = await submitRes.text().catch(() => "");
  let submit: SubmitResponse = {};
  try {
    submit = submitText ? (JSON.parse(submitText) as SubmitResponse) : {};
  } catch {
