...
- **Editor** with live mobile/desktop preview, autosave drafts, publish/unpublish, "Preview on store" (draft inside your real theme), per-field image upload / library / AI generation, translations tab (DeepL or Claude one-click), commerce tab (product & variant picker, discount code with Shopify check, gift add-ons, add-to-cart behaviour, UTM passthrough, live prices, per-market overrides).
