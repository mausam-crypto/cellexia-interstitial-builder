...
- **Section library** (20 types): listicle hero, numbered reason, differentiation text, purity icon row, numbered science explainer, clinical evidence with citation slots, four-pillar grid, expert quote, three-card pricing (center card enlarged + badged, strikethrough compare-at, per-unit math, optional gift line — not used), guarantee, checkmark comparison table, results timeline, testimonial carousel, review carousel, doctor FAQ accordion, final CTA — plus generic rich text / image / CTA / optional promo strip. Sticky mobile CTA bar and a locked disclaimer are page-level. The theme supplies the announcement bar, header and footer — the app never renders its own.
- **Editor** with live mobile/desktop preview, autosave drafts, publish/unpublish, "Preview on store" (draft inside your real theme), per-field image upload / library / AI generation, translations tab (DeepL or Claude one-click), commerce tab (product & variant picker, discount code with Shopify check, optional add-ons (unused — no free gift on the interstitials), add-to-cart behaviour, UTM passthrough, live prices, per-market overrides).
- **One-click images**: the Images tab (editor) / Images step (wizard) lets Claude pre-write one prompt per image slot from the page's copy — consistent protagonist, real-looking testimonial portraits, authentic moments, no fake before/afters or readable packaging — then generates every slot with Higgsfield (diagrams via Claude SVG), 3 in parallel, straight into the page. Real portraits, packshots and icons are never auto-generated.
- **Subscription mode** (native Shopify selling plans): a page can be subscription-only — cards = variant + selling plan (delivery frequencies), buttons add `selling_plan`, live subscriber prices from the plan allocations (incl. intro/trial "first delivery … then …"), editable presets (delivery/offer lines, terms disclosure, FAQ, benefits row, "Subscribe & save"), market fallback (one-time button or hide card).
- **Prompts page**: every prompt behind the image pipeline (prompt-writer system prompt, its message template with placeholders, the SVG diagram prompt, the brand style suffix, per-slot default prompts) is editable in the admin with reset-to-default.
- **After add to cart** (Settings, on by default): pricing buttons add the items to the cart and land on the **Shop All** collection with the **cart drawer open** (encourages adding more products) — or straight to checkout / cart page, store-wide or per page.
- **Store header** always shows by default (the theme's real announcement bar/header/footer wrap every page and preview); it can be hidden globally or per page.
...
3. **Deploy the config + theme helper**: `shopify app deploy` (pushes scopes, webhooks, the **App Proxy** `[app_proxy] prefix=a subpath=go url=https://<app>/proxy`, and the theme app extension `extensions/cellexia-interstitial-helpers` — an app embed that opens the theme's cart drawer after an interstitial add-to-cart). Verify in the Dev Dashboard → App → Configuration → App proxy. Then in Shopify admin: Online Store → Themes → Customize → **App embeds** → enable **"Open cart after add"** → Save (works on the vintage theme; no theme files are edited).
